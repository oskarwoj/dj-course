...
import { getPointTypeBgClass, getPointTypeBadgeClass, ALL_POINT_TYPES } from '../utils/point-type.utils';
      ...
      {route.points.length > 0 ? (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-gray-50 rounded-lg">
            ...
            <div className="text-center">
              <div className={`text-2xl font-bold capitalize ${getStatusColor(route.status)}`}>
                {route.status}
              </div>
              ...
              {ALL_POINT_TYPES.map((type) => {
                ...
                return (
                  <div key={type} className="flex justify-between">
                    ...
                    <span className={`font-medium ${colors[type]}`}>{count}</span>
                  </div>
                );
              })}
            </div>
                ...
                {route.points.map((point, index) => (
                  ...
                  >
                    <div className="flex-shrink-0">
                      <div
                        className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-sm font-medium ${getPointTypeBgClass(point.type)}`}
                      >
                        {index + 1}
                      </div>
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span
                          className={`px-2 py-1 text-xs rounded-full font-medium ${getPointTypeBadgeClass(point.type)}`}
                        >
