...
/**
 * Converts a driver's route to a shipment format for unified display
 */
export const convertDriverRouteToShipment = (driverRoute: DriverRoute, driver: Driver): Shipment => {
  ...
  const routeData: RouteData = {
    ...
    status:
      driverRoute.status === 'active' ? 'active' : driverRoute.status === 'completed' ? 'completed' : 'planned',
...
/**
 * Generates vehicle route shipments for a given vehicle
 */
export const generateVehicleRouteShipments = (vehicle: VehicleType): Shipment[] => {
  const routes: Shipment[] = [];
  const now = new Date();

  // Generate sample routes for the vehicle
  for (let i = 0; i < 8; i++) {
...
/**
 * Creates a default route for route planning mode
 */
